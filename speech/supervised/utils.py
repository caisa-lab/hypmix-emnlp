...
#     gain1 = cupy.max(compute_gain((sound1.data), fs))  # Decibel
#     gain2 = cupy.max(compute_gain((sound2.data), fs))
...
def mix(sound1, sound2, r, fs):
    # print("Here in mix")
    gain1 = cupy.max(compute_gain(cp.asarray(sound1.data), fs),axis=1)  # Decibel
    gain2 = cupy.max(compute_gain(cp.asarray(sound2.data), fs),axis=1)
    t = 1.0 / (1 + cupy.power(10, (gain1 - gain2) / 20.) * (1 - r) / r)
...
def mixup_sound(out, target_reweighted,r,fs):
    ...
    r = r[:out.shape[0]]
    sound = mix(out,out[indices[0:out.shape[0]]],r,fs)
    target_shuffled_onehot = target_reweighted[indices[0:out.shape[0]]]
